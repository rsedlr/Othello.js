...
function updateBoard(pass = false) {
  ...
  board.forEach((row, r) => {
    row.forEach((square, c) => {
      ...
      if (board[r][c] != boardBackup[r][c] && pass == false) {
        // || board[r][c] == ' '
      ...
      if (board[r][c] == ' ' && direction.reduce((a, b) => a + b) != 0) {
        ...
        gameOver = false
        if (gameMode != 2) {
          var score = direction.reduce(function (a, b) {
            return a + b
          }, 0)
        ...
        if (gameMode == 2 || (gameMode == 1 && player == 'b'))
          boardSquare.onclick = () => placePiece(r, c, direction)
      }
    })
  })
  var find = findTotal()
    ...
    if (gameMode == 1) {
      ...
      if (player == 'w')
        move = setTimeout(function () {
          placePiece(
            ...idealMove[1][Math.floor(Math.random() * idealMove[1].length)].slice()
          )
          waiting = false
        }, 700)
    } else if (gameMode == 0) {
      ...
      move = setTimeout(function () {
        ...
        waiting = false
      }, delay.options[delay.selectedIndex].value)
    } else if (gameMode == 2) {
...
function findTotal(end = false) {
  ...
  if (
    whiteTotal + blackTotal == parseInt(rowDrop.value) * parseInt(colDrop.value) ||
    whiteTotal == 0 ||
    blackTotal == 0 ||
    end
  ) {
